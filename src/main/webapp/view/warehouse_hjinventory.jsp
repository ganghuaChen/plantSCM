...
<head>
    ...
    <script>
		   ...
		   "click #TableSave":function(e,value,row,index){
			     $("body").delegate('#tb_departments tr .save', 'click', function () {
	            	var obj =$(this).parent().parent().children(".editable")
	  				...
	  				var hjFactory=hjFactory;
	  				var hjActualNumber=obj.eq(0).find("input").val();
	  			  ...
	  			  if(isValid(newData)){	  				  
	  				//给后台发送ajax请求，修改数据
	  				  $.ajax({
		                      type:"POST",
		                      url:"/plantSCM/WarehouseInventory/modifyHjInventoryRecord",
       ...
       function isValid(newData){           	
		    if(  !isNaN(newData["hjActualNumber"])&&!isNull(newData["hjActualNumber"])&&newData["hjActualNumber"]>=0&& isInteger(newData["hjActualNumber"])
		    		){		    	
		    	return true;
...
<body>
          ...
          <header>
            ...
            <li style="background-color:#F0FFFF;"><a href="javascript:void(0)" onclick="clickhjinventory()">焊剂库存</a></li>
