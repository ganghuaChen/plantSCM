...
<meta name="viewport" content="width=device-width, initial-scale=1">
  ...
  //获取焊剂厂家
   function getHjFactory(){
    	$.ajax({
            type:"GET",
            url:"/plantSCM/WarehouseIn/getHjFactory",
    ...
    $(function () {
     ...
     //初始化日期选择器
    	...
    	$.ajax({
            type:"get",
            url:"/plantSCM/WarehouseIn/getHjInInitRecord",///初始化页面，默认显示未入库的
            dataType:"json",
            success:function(data){  
            	for(i in data.result) 
	            {
	               dataarray[i]={"id":data.result[i].id,"hjDate":data.result[i].hjDate,"hjSpecies":data.result[i].hjSpecies,"hjPrice":data.result[i].hjPrice,"hjFrom":data.result[i].hjFrom,"hjNumber":data.result[i].hjNumber,"hjTotalPrice":data.result[i].hjTotalPrice,"hjStorageState":data.result[i].hjStorageState,"hjNote":data.result[i].hjNote};
	               if(dataarray[i].hjStorageState == false){
	            	   dataarray[i].hjStorageState = "未入库";
	               }   
	               else if(dataarray[i].hjStorageState == true){
	            	   dataarray[i].hjStorageState = "已入库";
	               }
...
function clicksearch(){
	   var hjStorageState = $("#select1").find("option:selected").text();//状态
	   
	   var startTime =  $("#datetimepicker1").find("input").val();//开始时间
       var endTime = $("#datetimepicker2").find("input").val();//结束时间
       var hjFrom = $("#select2").find("option:selected").text();
       if(isNull(startTime) || isNull(endTime)){
    	   toastr.info("请选择时间");
       }else{
    	   var temp={
    			   hjStorageState:hjStorageState,
    			   startTime:startTime,
    			   endTime:endTime,
    			   hjFrom:hjFrom
    	   };    	  
   ...
   function search(temp){
	   var hjStorageState = $("#select1").find("option:selected").text();//状态
	   if(hjStorageState == "未入库"){
		   hjStorageState=false;
	   }
	   else if(hjStorageState == "已入库"){
		   hjStorageState=true;
	   }
	   var temp={
			   hjStorageState:hjStorageState,//状态
			   startTime:$("#datetimepicker1").find("input").val(),//开始时间
			   endTime:$("#datetimepicker2").find("input").val(),//结束时间
			   hjFrom:$("#select2").find("option:selected").text()
	   };
	   //将搜索条件传到后台
	   $.ajax({
		   type:"GET",
 	        url:"/plantSCM/WarehouseIn/getHjInRecord",
 	        ...
 	        success:function(data){
 	        	...
 	        	if(data.code==0){
            		...
            		for(i=0;i<result.length;i++){
            			if(result[i].hjStorageState == true){
            				result[i].hjStorageState = "已入库";
            			}
            			else if(result[i].hjStorageState == false){
            				result[i].hjStorageState = "未入库";
            			}
            		}
   ...
   function hjInAck(){
	   ...
	   if(ackData.length <= 0){
		   ...
		   for(var i=0;i<ackData.length;i++){
			   if(ackData[i].hjStorageState == "未入库"){
				   ackData[i].hjStorageState = false;
				   dataarray.push(ackData[i]);
			   }
			   else if(ackData[i].hjStorageState == "已入库"){
				   toastr.info("该记录已入库");
				   break;
			   }
		   }		   
		   if(dataarray.length>0){
			   $.ajax({
				   type:"POST",
				   url:"/plantSCM/WarehouseIn/confirmHjInRecord",
				   contentType:"application/json;charset=UTF-8",
...
<!--水平导航栏-->
          <header>
            ...
            <li  style="background-color:#F0FFFF;"><a href="javascript:void(0)" onclick="clickhjinrecord()">焊剂入库记录</a></li>
            </ul>
            </li>
            <li>
...
<div class="row">
          <header>
                <nav class="navbar navbar-default fixed" id="header-nav" role="navigation" style="width:100%">
                    <div class="container-fluid">
                        <div class="navbar-header">
                            <a class="navbar-brand" href="#">焊剂入库记录</a>
                        </div>                                          
                    </div>
                </nav>
            </header>         
        ...
        <div style="white-space:pre;float:left;">状态</div>
            <div style="float:left;">
              ...
              <div style="float:left;">
                 ...
                 <div style="float:left;">
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <button id="rukubutton"  type="button" class="btn btn-primary"  onclick="hjInAck()">确认入库</button>          
                 </div>    
            <!-- 搜索栏结束 -->       
             </div><!-- 一行row结束 -->
